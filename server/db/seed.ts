...
// sections, brief and statusUpdates, which supabase-schema.sql omitted. Date values are
// kept as Date expressions (not converted to ISO strings) so the fixtures are checked
// against Project[] at compile time — entityToRow accepts Date objects natively.
...
// Copied verbatim from services/enhancedApi.ts:227-431 — keep every field, including
// blockedBy and blocking, which supabase-schema.sql omitted. Date values are kept as
// Date expressions so the fixtures are checked against Task[] at compile time.
// (Declared const, not let, since this file never reassigns it — the original mutates
// it elsewhere in enhancedApi.ts for demo runtime updates, which is out of scope here.)
const TASKS: Task[] = [
    { 
      id: 'task-1', 
      projectId: 'proj-1', 
...
// insertAll is generic so the fixture arrays above can stay typed as Project[]/Task[]
// (checked at compile time against types.ts) while still being handed to entityToRow,
// which operates on Record<string, unknown>. The cast here is a boundary adapter for
// the generic serialization helper, not a widening of the fixture data's own types.
function insertAll<T extends object>(db: Database.Database, table: string, rows: T[], spec: FieldSpec) {
  for (const entity of rows) {
    const row = entityToRow(entity as Record<string, unknown>, spec);
